...
One thing the macro does **not** do for you: check `source_chain`/`source_address` against an allowlist of counterpart contracts you trust. Axelar authenticates _that_ the message came from that sender, not _whether_ you should listen to them — do that check first thing in `__execute`.
