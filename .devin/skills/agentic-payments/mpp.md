  ...
  methods: [
    stellar.charge({
      ...
      feePayer: process.env.FEE_PAYER_SECRET
        ? {
            envelopeSigner: StellarSdk.Keypair.fromSecret(
...
app.listen(3002, () =>
  console.log("MPP charge server on http://localhost:3002"),
);
```
...
function resolveRecipient() {
  ...
  if (raw.startsWith("S")) {
    ...
    try {
      ...
      console.error(
        "STELLAR_RECIPIENT looks like a secret key but failed to parse — disabling MPP",
      );
      return "";
    }
  }
  ...
  if (!StrKey.isValidEd25519PublicKey(raw)) {
    console.error(
      `STELLAR_RECIPIENT is not a valid Stellar public key — disabling MPP: ${raw.slice(0, 8)}...`,
    );
    return "";
  }
...
if (RECIPIENT && process.env.MPP_SECRET_KEY) {
  ...
  console.warn(
    "MPP_SECRET_KEY missing, or STELLAR_RECIPIENT missing or invalid — MPP charge middleware disabled",
  );
}
...
const chargeMppx =
  RECIPIENT && process.env.MPP_SECRET_KEY
    ? Mppx.create({
        methods: [stellar.charge({ recipient: RECIPIENT /* ... */ })],
      })
    : null;
...
if (process.env.MPP_COMMITMENT_KEY) {
  if (/^[0-9a-f]{64}$/i.test(process.env.MPP_COMMITMENT_KEY)) {
    ...
    console.error(
      "MPP_COMMITMENT_KEY is not 64 hex characters — Session mode disabled, Charge unaffected",
    );
  }
}
...
if (process.env.FEE_PAYER_SECRET) {
  if (StrKey.isValidEd25519SecretSeed(process.env.FEE_PAYER_SECRET)) {
    ...
    console.error(
      "FEE_PAYER_SECRET is not a valid Stellar secret key — Session mode disabled, Charge unaffected",
    );
  }
}
...
if (process.env.MPP_CHANNEL_CONTRACT) {
  if (StrKey.isValidContract(process.env.MPP_CHANNEL_CONTRACT)) {
    ...
    console.error(
      "MPP_CHANNEL_CONTRACT is not a valid Stellar contract ID — Session mode disabled, Charge unaffected",
    );
  }
}
...
> The bare `mppx` root does **not** export `Mppx` at all — always import it from the subpaths above. (`Store` _is_ re-exported from the root and is the same object as `mppx/server`'s, but importing it from `mppx/server` keeps the server imports together.)
...
**Session mode only:** 4. Deploy the one-way-channel contract (see [stellar-mpp-sdk](https://github.com/stellar/stellar-mpp-sdk) for deploy script) 5. Generate a 64-char hex ed25519 seed for the commitment key — this value
