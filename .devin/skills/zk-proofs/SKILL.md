...
**Both curves have base host functions since Protocol 25** — the curve no longer decides everything. What still matters is the proof _system_: Groth16 verifies with a small contract on either curve, while UltraHonk needs a dedicated in-contract verifier (one exists — see the Noir walkthrough — but it relies on CAP-0080 host functions, so Protocol 26+).
...
3. **Attestation oracle** (fallback for older protocol targets or unaudited-verifier concerns): a service runs `bb verify` off-chain and submits a signed attestation; the contract `require_auth()`s the attester and applies policy. The trust assumption (the attester) must be explicit and documented — this is _not_ trustless ZK, it's a verifiable-computation oracle.
...
- **Verifying the proof but not the statement.** A valid proof only shows _some_ witness satisfies the circuit. The contract must validate the public inputs' semantics: who is this proof for, which Merkle root, which action, which amount.
